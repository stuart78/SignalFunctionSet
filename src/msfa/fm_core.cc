...
// SFS addition: bitmask of carrier ops for an algorithm (see fm_core.h).
int fmCoreCarrierMask(int algorithm) {
  if (algorithm < 0 || algorithm > 31) return 0;
  int m = 0;
  for (int op = 0; op < 6; op++)
    if ((algorithms[algorithm].ops[op] & 7) == OUT_BUS_ADD) m |= (1 << op);
  return m;
}
