...
// SFS: scales the per-step increment so envelope times track the host SR.
// 1.0 at 44.1k (the rate the msfa constants were calibrated against).
static double s_envSrFactor = 1.0;
...
void Env::advance(int newix) {
  ...
  if (ix_ < 4) {
    ...
    inc_ = (int)((double)inc_ * s_envSrFactor);  // SFS: sample-rate correction
    if (inc_ < 1) inc_ = 1;
  }
}
