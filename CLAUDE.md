...
32. **Slice** (slug `Slice`) — *(WIP, hidden)* Cuts a stereo stream onto a grid and reworks each piece (21HP). Thirty seconds of audio go into a circular buffer; a grid of slices runs over it, 10ms to 1s each, free-running or measured from CLOCK (with DIV). You **pick a PATTERN** and it governs all playback: a fixed sequence of what to do to each slice, drawn from seven transforms (**cut · swap · delay · shuffle · reverse · repeat · pitch**) with `-1` for "leave this one alone". Twelve of them — Straight, Gate 2/4, Ping-pong, Reverse 2, Echo, Stutter, Dive, Scatter, Tumble, Glitch, Random. **The first cut rolled seven per-slice weights and it was wrong**: everything came out as the same undifferentiated scatter and no setting was a thing you could learn or return to. Random is now one entry among twelve rather than the whole design. DEPTH crossfades the altered slice against the straight one, which is the only thing left for "how strong" to mean once a pattern is doing the choosing. **It is zero-latency when it passes through**, and that shapes everything: a straight slice is read from the write head sample for sample, but you cannot reverse a slice you are still recording, so the transforms that need a finished one reach back a slot and work on THAT (reverse plays the previous slice backwards, pitch plays it slowed). The alternative — delaying everything by one slice so each transform acts on "this" one — is a second of latency at a one-second slice. **REPEAT is the exception and stays live**: it plays the first 1/N of the slice as it arrives, then loops the fragment it just captured. **The edge fade is not a grain window**: each slice fades in and out over a few ms and runs at unity between, so at DEPTH 0 the module is transparent and can sit on a bus — enveloping the whole slice (which is what a Hann window means in granular synthesis) would amplitude-modulate at the slice rate whether or not anything was transformed, and that is a tremolo, not a slicer. The whole-slice form is a menu option. The Random pattern is still **seeded**, so a take reproduces from a reset and RESEED means something. (Verified against the real table: every fixed pattern plays its stated figure and repeats exactly at its own length over 400 slices.) **BAR quantizes REACH** — how far back delay and shuffle go — to whole bars, because "one bar ago" lands and "1.37 seconds ago" does not. **LINK** pairs the channels (default) or gives the right its own source, which decorrelates the image. FREEZE stops the write head and loops the buffer. The panel departs from the plugin's pot-over-jack pairs: **screen on top, controls in the middle, every jack in two rows at the foot** with the outputs on a plate — Slice has ten inputs and only three modulate a knob, so pairing would scatter the audio and clock jacks among the controls.
