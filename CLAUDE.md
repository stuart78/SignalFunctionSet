...
30. **Loom** (slug `Loom`) — *(WIP, hidden)* Eight-string resonator (28HP). Each string is a digital waveguide: delay loop for pitch, in-loop lowpass that eats the treble first, a 4-section allpass chain for stiffness/inharmonicity, and a comb on the **output tap** for pluck position. Filter phase delay is evaluated **exactly at the string's own fundamental**, not at DC, or the strings run sharp. **EXCITE is a continuous axis, not a switch** — HAMMER · PLUCK · BOW · WIND as nodes, crossfading between adjacent pairs; at a pure node the code path (including how many random numbers it draws) is identical to the single-exciter version, so the strike tones stay exactly as tuned. **Bow**: Smith friction table, and the harshness control is the string-velocity feedback — high values drive the limit cycle **chaotic**, filling the gaps between partials with broadband noise at −11 dB (it sounded bright; it was noisy). At 1.5 the inter-harmonic floor is −47 dB. Bow speed sets loudness, not whether it speaks (a velocity-scaled drive left gentle bowing silent), and a little **hair noise** is load-bearing: without it the string locks into its sub-octave at the top of the range. **WIND** is an aeolian harp — vortex shedding drives a *narrow* band and the instrument sings on upper partials (fundamental ~29 dB down); the first version fed it white noise, which is just hiss through a comb. **COUPLE**: every string's *motion* (not its combed pickup signal — that comb peaks at +6 dB and howls inside a feedback loop) feeds a shared bridge bus, and each string trades a band-limited fraction, `x += k·(bus − ownLowpassed)`. Only the transmitted band is damped, so the string keeps its brightness; the in-phase mode has gain 1, so it is unconditionally stable. Ceiling is **0.06** — the first version's 0.35 cost 89% of the sustain at max and bought *less* sympathetic ring, because it killed the driving string before it could transmit. Display is the instrument: PLAY tab strums with the mouse (Y = where along the string you caught it), the other six tabs edit one attribute per string by height. 8 gate ins + poly VEL, 8 string outs + stereo mix, 12 auto-play arpeggio patterns.
