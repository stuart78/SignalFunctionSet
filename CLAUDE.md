...
31. **Key** (slug `Key`) — *(WIP, hidden)* Quantizer that takes its key from the patch (10HP). Reads **ROOT** (1V/oct, semitone-quantized) and **SCALE** (1V per scale) in the plugin convention, so one key change travels to Arrange, Note, Chance, Muse, Fugue, Loom and this together instead of being set by hand in each. Four channels, each **polyphonic** (16ch), all sharing one key — a whole voice group stays in agreement rather than four quantizers drifting apart. Per channel: IN, **OFFSET** (scale *degrees* by default, so +2 moves two steps up the scale and stays in key; semitones via the menu), **CHG** trigger on note change, OUT. Global **TRIG** turns it into a sample-and-hold. **The quantizer works against the canonical table's real float intervals, not a 12-bit pitch mask** — Harmonic series, Pelog and Slendro are genuinely not 12-TET, and a mask would silently round them onto the chromatic grid and throw away the thing that makes them themselves; only the on-screen keyboard rounds, having just twelve keys to draw on. Clicking a key forks a **custom mask** off the current preset (selecting a scale again wins back). Hysteresis (menu, default 12 cents) stops a pitch sitting on a boundary chattering; it is abandoned whenever the key changes, or a held note would survive outside the new scale. Rounding: nearest / down / up.
