...
30. **Loom** (slug `Loom`) — *(WIP, hidden)* Eight-string resonator (28HP). Each string is a digital waveguide: delay loop for pitch, in-loop lowpass that eats the treble first, a 4-section allpass chain for stiffness/inharmonicity, and a comb on the **output tap** for pluck position. What makes it one instrument is the **bridge**: every string's *motion* (not its combed pickup signal — that comb peaks at +6 dB and howls inside a feedback loop) sums into a shared bus, and COUPLE **mixes** a fraction back into each loop, `x = x(1−k) + k·bus`, so a string gives up exactly what it takes. Adding instead of mixing has no stable setting. Four exciters: pluck / hammer (bipolar velocity pulse — a one-sided bump is a DC step, and a delay loop passes DC at unity) / bow (Smith friction table; the four constants were swept against the real loop, since a shallow curve settles at a silent DC fixed point and the wrong balance locks the sub-octave) / aeolian. Filter phase delay is evaluated **exactly at the string's own fundamental**, not at DC, or the strings run sharp. Display is the instrument: PLAY tab strums with the mouse (Y = where along the string you caught it), the other six tabs edit one attribute per string by height. 8 gate ins + poly VEL, 8 string outs + stereo mix, 12 auto-play arpeggio patterns.
